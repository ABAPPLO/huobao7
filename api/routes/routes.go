...
func SetupRouter(cfg *config.Config, db *gorm.DB, log *logger.Logger, localStorage interface{}) *gin.Engine {
	...
	api := r.Group("/api/v1")
	{
		...
		episodes := api.Group("/episodes")
		{
			...
			episodes.POST("/:episode_id/batch-first-frame-images", framePromptHandler.BatchGenerateFirstFrameImages)
			...
			episodes.DELETE("/:episode_id", dramaHandler.DeleteEpisode)
		}
		...
		videos := api.Group("/videos")
		{
			...
			videos.POST("/keyframe-prompts", videoGenHandler.GenerateKeyframeVideoPrompts)
			videos.POST("/keyframe-sequence", videoGenHandler.GenerateKeyframeSequenceVideos)
		}
